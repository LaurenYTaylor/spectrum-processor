...
def test_flux_one():
  flux1=spec.flux[0]
  np.testing.assert_approx_equal(flux1=12.10058), "ya yeet"
  
#Better to be safe than sorry
def test_flux_two():
    flux2=spec.flux[1]
    np.testing.assert_approx_equal(flux2=-0.825348), "ya yeet"

#Lambda testing-determines if we can read wavelength
def test_lambda_one():
      lambda1=spec.loglam[0]
      np.testing.assert_approx_equal(lambda1=3.5533), "ya yeet"
        
def test_lambda_two():
      lambda2=spec.loglam[1]
      np.testing.assert_approx_equal(lambda2=3.5534), "ya yeet"
      
#Ra test-Pretty sure  you get the point
def test_ra_one():
      ra1=spec.ra[0]
      np.testing.assert_approx_equal(ra1=31.408747), "ya yeet"
